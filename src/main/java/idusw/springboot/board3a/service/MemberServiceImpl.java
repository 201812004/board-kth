...
public class MemberServiceImpl implements MemberService{
    ...
    public List<Member> readList() {
        // 1. 매개변수를 처리하고(처리를 위항 객체 선언 및 초기화), 리파지터리 객체에게 전달
        // 2. 리파지터리 객체의 해당 메소드가 처리한 후 결과를 반환 유형으로 변환
        // repository : entity를 대상으로함, service : dto or domain 객체를 대상으로 함
    ...
    @Override
    public Member login(Member m) { // 인터페이스에 선언된 메소드 중 구현하지 않은 메소드를 구현해야 함.
        Member result = null;
